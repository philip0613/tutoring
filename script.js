...
function renderQuestionList(questionsRaw, containerElement, isAdmin = false) {
    ...
    questions.forEach(q => {
        const li = document.createElement('li');
        li.style.position = 'relative'; li.style.marginBottom = '15px';
...
// 💡 [사진 요구사항 완벽 반영] 피드백 아코디언 스타일을 적용한 시험지 리스트!
function renderAdminExamList(exams, containerElement) {
    ...
    exams.forEach(e => {
        ...
        const titleBtn = document.createElement('button');
        titleBtn.className = 'feedback-title-btn'; // 기존 피드백 버튼 CSS 완벽 호환
        titleBtn.innerHTML = `📝 ${e.exam_title} : <strong>${e.score}점</strong>`;

        const detailDiv = document.createElement('div');
        detailDiv.className = 'feedback-detail'; // 기존 피드백 펼침 CSS 완벽 호환

        // DB 컬럼이 꼬였을 경우를 대비해 여러 변수명 체크
        ...
        detailDiv.innerHTML = hasImg 
            ? `<img src="${imgUrl}" alt="시험지 원본" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" style="max-width:100%; border-radius:4px; margin-top:10px; display:block;">
...
async function loadTeacherDashboard() {
    ...
    } catch (error) { console.error('학생 목록 에러:', error); }
}

async function loadStudentDetail(studentId) {
    try {
        const examRes = await fetch(`/api/getExams?studentId=${studentId}&student_id=${studentId}`);
        renderAdminExamList(await examRes.json() |> extractDataArray, document.getElementById('examListAdmin'));
    ...
    } catch (error) { console.error('상세정보 로드 에러:', error); }
}

async function loadStudentDashboard() {
    try {
        const examRes = await fetch(`/api/getExams?studentId=${currentUser.id}&student_id=${currentUser.id}`);
        renderStudentExamList(await examRes.json() |> extractDataArray, document.getElementById('myExamList'));
    ...
    } catch (error) { console.error('대시보드 에러:', error); }
}

// 💡 파이프라인 연산자 대안 (위쪽 화살표 함수 에러 방지용으로 안전하게 수정)
async function loadStudentDetail(studentId) {
    try {
        const examRes = await fetch(`/api/getExams?studentId=${studentId}&student_id=${studentId}`);
...
function checkSession() {
    ...
    if (sessionData) {
        ...
        document.getElementById('userGreeting').textContent = `${currentUser.name || currentUser.id}님 환영합니다!`;
...
async function handleAnswerQuestion(questionId) {
    const text = document.getElementById(`ansText_${questionId}`).value.trim();
    ...
    try {
        ...
        if (res.ok) { alert('✅ 답변 성공!'); loadStudentDetail(currentStudentId); } 
        else { alert('답변 등록 실패'); }
    } catch (err) { alert('에러: ' + err.message); } finally { btn.innerText = "답변 등록하기"; btn.disabled = false; }
}

async function handleCreateStudent() {
    const name = document.getElementById('newStudentName').value.trim();
    const loginId = document.getElementById('newStudentId').value.trim();
    if (!name || !loginId) return alert('학생 이름과 아이디를 입력하세요.');
    try {
        ...
        if (res.ok) { alert('✅ 생성 완료!'); loadTeacherDashboard(); } 
        else { alert('생성 실패'); }
    } catch (err) { alert('에러: ' + err.message); }
}

async function handleUploadExam() {
    if (!currentStudentId) return alert("학생을 선택해주세요!");
...
async function handleSendFeedback() {
    const title = document.getElementById('feedbackTitleAdmin').value.trim();
    const text = document.getElementById('feedbackTextAdmin').value.trim();
    if (!title || !text) return alert('모두 입력해주세요!');
    try {
        ...
        if (res.ok) { alert('✅ 전송 완료!'); loadStudentDetail(currentStudentId); }
    } catch (error) { console.error('전송 에러:', error); }
}

async function handleDeleteStudent() {
    if (!currentStudentId || !confirm('🚨 정말 삭제하시겠습니까?')) return;
    try {
        const res = await fetch('/api/adminAction', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'deleteStudent', student_id: currentStudentId, studentId: currentStudentId }) 
        });
        if (res.ok) { alert('✅ 삭제 완료'); document.getElementById('studentManagePanel').classList.add('hidden'); loadTeacherDashboard(); }
    } catch (err) { alert('에러: ' + err.message); }
}
...
async function handleAskQuestion() {
    ...
    try {
        ...
        const res = await fetch('/api/askQuestion', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ student_id: userId, studentId: userId, id: userId, userId: userId, question_text: text, questionText: text, image_base64: image_base64, image_name: image_name })
        });
        if (res.ok) { alert('✅ 등록 완료!'); loadStudentDashboard(); } 
        else { alert('등록 실패'); }
    } catch (err) { alert('에러: ' + err.message); } 
...
async function handleChangePassword() {
    ...
    if (!newPw || newPw.length < 6) return alert('최소 6자리 이상이어야 합니다.');
    try {
        const res = await fetch('/api/changePassword', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: currentUser.id, newPassword: newPw })
        });
        if (res.ok) { alert('✅ 변경 완료! 다시 로그인해주세요.'); handleLogout(); }
    } catch (err) { alert('에러: ' + err.message); }
}
